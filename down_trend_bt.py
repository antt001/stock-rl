...
for ticker in tickers:
    ...
    df['BBU'], df['BBM'], df['BBL'] = talib.BBANDS(
    ...
    if valid_index_for_signal.empty:
        print(f"No valid data points found for ticker {ticker}.")
        continue
    ...
    plot_file = os.path.join(output_dir, f"{ticker}_{start_date}_{end_date}.png")
    fig.savefig(plot_file)
    plt.close(fig)
    print(f"Saved plot: {plot_file}")
    print(f"Backtesting complete for ticker: {ticker}")
